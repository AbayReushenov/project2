...
// Используем классы для создание карточек меню
    ...
    const prev = document.querySelector('.offer__slider-prev'),
        ...
        slide4 = document.querySelectorAll('.offer__slide');

    let slideIndex = 1;

    const total = slide4.length,
        nN = n => n > 9 ? "" + n: "0" + n,
        elementSpanTotal = document.createElement('span'),
        elementSpanCurrent = document.createElement('span');
    
    elementSpanTotal.innerHTML = `
          <span id="total">${nN(total)}</span>
          `;
    spanTotal.replaceWith(elementSpanTotal);

    function showAndHideSlide(whatNumberToSee) {

        slide4.forEach( (item, i) => {
            if ( i + 1 === whatNumberToSee) {
                item.classList.add('show');
                item.classList.remove('hide');

                elementSpanCurrent.innerHTML = `
                <span id="current">${nN(whatNumberToSee)}</span>
                `;
                spanCurrent.replaceWith(elementSpanCurrent);
